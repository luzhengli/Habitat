...
安全语义未改变。侧栏 Recovery 功能已完成三份同范围静态原型，统一采用“逐项恢复原用户
入口、保留 Store canonical 与项目链接”的安全合同；生产 React/Tauri 实现等待产品负责人
从账本、事务时间线和引导恢复三种方向中明确选择。M9 仍继续等待发布目标、签名/分发方式和
发布范围确认。
